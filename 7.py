...
for line in lines:
    ...
    for b in r.split(","):
        ...
        if not m:
            ...
            other_bags.append(m.groups())
    for b in other_bags:
        bags[l] = bags[l] + [b]
    # print(l, other_bags)
#print(bags)
cnt = 0
to_check = set(["shiny gold"])
checked = set()
...
class node:
    def __init__(self, name):
        ...
        self.parents = set()
    def __str__(self):
        return self.name + " " + str(self.children) + " " + str(self.parents)
...
for b in bags:
    for ob in bags[b]:
        nodes[b].children.add(ob)
        if ob[1] not in nodes:
            nodes[ob[1]] = node(ob[1])
        nodes[ob[1]].parents.add(b)

# for k, n in nodes.items():
#     print(n.name, n.children, n.parents)
...
def summul(name, ko):
    ...
    for k, c in n.children:
        # print(k,c)
        sum += ko * summul(c, int(k))
    return sum


print(summul("shiny gold", 1) - 1)