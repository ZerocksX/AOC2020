...
for line in lines:
    if len(line) == 0:
        has = 0
        if check(ob, 'byr', lambda x: int(x) >= 1920 and int(x) <= 2002):
            has += 1
        if check(ob, 'iyr', lambda x: int(x) >= 2010 and int(x) <= 2020):
            has += 1
        if check(ob, 'eyr', lambda x: int(x) >= 2020 and int(x) <= 2030):
            has += 1
        if check(ob, 'ecl', lambda x: x in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']):
            has += 1
        if check(ob, 'pid', lambda x: len(x) == 9):
            has += 1
        if check(ob, 'hgt', hgt):
            has += 1
        if check(ob, 'hcl', lambda x: len(x) == 7 and x[0] == '#' and ho(x[1:], hexs)):
            has += 1
        if has == len(keys):
            res += 1
        ob = {}
