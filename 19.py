...
for ln, line in enumerate(lines):
    ...
    if r[0] == '"':
        ...
        rls = []
        for r in r.split(" | "):
            rls.append([int(n) for n in r.split(" ")])
            for n in [int(n) for n in r.split(" ")]:
                parents[n] = parents[n] + [i]
        rules[i] = rls
...
def is_fit(expr, posrules, pos, cw = -1, indent=0):
    # print("#" * indent, posrules, pos, cw)
    # print(" " * indent, posrules, pos, cw)
    # print(" " * indent, expr, expr[pos:pos+1])
    if isinstance(posrules, str):
        if pos < len(expr) and expr[pos] == posrules:
            return True, pos+1
        return False, pos
    else:
        for rule in posrules:
            ...
            ppos = pos
            for r in rule:
                # print(" " * indent, "try rule:", rule, "r:", r, "-", rules[r], "pos:", pos)
                f, p = is_fit(expr, rules[r], pos, r,indent+1)
                # print(" " * indent, "rule result:", f, p)
                if f:
                    pos = p
                else:
                    m = False
